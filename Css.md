...
#### 清除浮动，避免父元素高度塌陷
当子元素都设置了 float 后，父元素可能“包不住”它们。让父元素形成 BFC 后，父元素就能正确计算这些浮动子元素的高度。

#### 阻止外部浮动元素影响内部布局
普通块元素可能会被旁边的浮动元素挤压；一旦元素建立了 BFC，它内部的布局会成为一个相对独立的区域，浮动交互被限制在这个上下文里。

#### 防止垂直 margin 合并
两个普通块级元素上下相邻时，垂直外边距可能会合并。建立 BFC 后，可以阻止父子之间或与外部之间一些不想要的 margin 合并。
