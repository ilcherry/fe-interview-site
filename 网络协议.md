...
## 从地址栏输入 url 发生什么
 1. 解析 URL 浏览器先把你输入的地址拆开，比如：

`https://www.example.com:443/path?a=1#top`
...
浏览器会先判断这是一个合法 URL，还是搜索词。

### 2. 查缓存

浏览器不会一上来就访问网络，而是先看本地有没有可用结果：
...
* 再查路由器或本地 DNS
* 最后去递归 DNS 服务器查
* 递归 DNS 再去问根域名、顶级域名、权威 DNS

最后拿到服务器 IP。

### 4. 建立 TCP 连接

如果是 HTTP/1.1 或 HTTP/2，通常要先建立 TCP 连接，也就是三次握手：
...
如果是 HTTPS，还没完，后面还要 TLS 握手。

### 5. TLS 握手（HTTPS）

如果是 `https`，浏览器会和服务器协商安全连接：
...
* CSS 会阻塞渲染
* 普通 `<script>` 可能阻塞 HTML 解析
* `defer`、`async` 会影响加载执行时机
...
* HTTP/1.1 支持 keep-alive
* HTTP/2 支持多路复用
* HTTP/3 基于 QUIC，底层不再是 TCP
...
如果服务器返回 301/302/307/308，浏览器会跳到新地址重新请求。
...
**解析地址 → 查 IP → 建连接 → 发请求 → 服务器返回资源 → 浏览器解析渲染页面。**
